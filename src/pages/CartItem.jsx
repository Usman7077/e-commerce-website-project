...
function CartItem({ item, handleCart, key }) {
  const { cart, setCart } = useContext(CartContext);
  //   const [showTrash, setShowTrash] = useState(false);
  function removeItem(sku) {
    const filterCart = cart.filter((item) => item.sku !== sku);
    setCart(filterCart);
  }
  return (
    <li
      className=" p-4 gap-x-4 grid grid-cols-[8rem_1fr] "
      key={key}
      //   onMouseEnter={() => setShowTrash((prev) => !prev)}
      //   onMouseOut={() => setShowTrash((prev) => !prev)}
    >
      <Link
        className="w-32 h-32 row-span-3"
        to={`/product/${item.sku}`}
        onClick={handleCart}
      >
        <img
      ...
      <Link to={`/product/${item.sku}`} onClick={handleCart}>
      ...
      <div className="flex justify-between text-2xl mt-4">
        ...
        <div
          className="h-6 w-6 hover:h-8 hover:w-8 transition-all cursor-pointer"
          //   {`${showTrash ? "h-8 w-8" : "h-0 w-0"}`}
